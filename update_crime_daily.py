...
# 🔧 ORTAK ENV — CRIME_DATA_DIR artık MUTLAK
env:
...
jobs:
  run_fr:
    ...
    steps:
      ...
      # (0) — CWD = repo kökü, path'ler ABSOLUTE; relative CWD etkilenmez
      - name: (0) Build FR daily (GRID & EVENTS) + Y_label
        ...
        run: |
          set -e
          echo "📂 CWD: $(pwd)"
          echo "🔧 ENV → FR_EVENTS_PATH: ${FR_EVENTS_PATH}"
          echo "🔧 ENV → FR_OUT_EVENTS : ${FR_OUT_EVENTS}"
          echo "🔧 ENV → FR_OUT_GRID   : ${FR_OUT_GRID}"
      ...
      - name: Run patrol planner
        run: |
          ...
          if not grid_path.exists():
              print(f"❌ GRID bulunamadı: {grid_path}", file=sys.stderr); sys.exit(2)
          df = pd.read_csv(grid_path)
          if df.empty:
              print("⚠️ GRID boş, plan üretilemedi.")
              pd.DataFrame(columns=["GEOID","date","risk_score","Y_label","events_count"]).to_csv(out_csv, index=False)
              sys.exit(0)
          need = ["GEOID","date"]
          for c in need:
              if c not in df.columns:
                  print(f"❌ Zorunlu kolon yok: {c}", file=sys.stderr); sys.exit(3)
          df["date"] = pd.to_datetime(df["date"], errors="coerce")
          last_day = df["date"].dropna().max()
          if pd.isna(last_day):
              print("❌ Tarih alanı hatalı.", file=sys.stderr); sys.exit(4)
