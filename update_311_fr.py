...
# update_311_fr.py
#
# 311 verisini indirir (Socrata), GEOID eşler, 3 saatlik özet çıkarır
# ve (varsa) 911 sonrası üretilmiş sf_crime_01.csv ile güvenli birleşim yapar.
# - Sadece izinli 311 alanları kullanılır
# - _x/_y suffix oluşumu engellenir
# - population, temp vs. gibi alanlar ÜRETİLMEZ/TAŞINMAZ
...
# adlandırma
RAW_311_NAME_Y = os.getenv("RAW_311_NAME_Y", "sf_311_last_5_years_y.csv")   # ham
AGG_BASENAME   = os.getenv("AGG_311_NAME",   "sf_311_last_5_years.csv")     # 3h özet
AGG_ALIAS      = os.getenv("AGG_311_ALIAS",  "sf_311_last_5_years_3h.csv")  # alias
...
def resolve_existing_raw_path():
    for cand in RAW_CANDIDATES:
        if os.path.exists(cand):
            print(f"🔎 Mevcut 311 _y CSV bulundu: {os.path.abspath(cand)}")
            return cand
...
def load_existing_raw_or_seed(raw_path: str) -> pd.DataFrame:
    ...
    if os.path.exists(base_csv):
        ...
        if not seed.empty:
            ...
            print(f"✅ Base CSV ham seed olarak işlendi ve {os.path.relpath(raw_path)} yazıldı.")
            return seed_geo
...
def main():
    ...
    # 5) 3 SAATLİK ÖZET (GEOID × date × hour_range)
    if not df_raw.empty:
        ...
        # yalnızca izinli kolonları yaz
        grouped = grouped[[c for c in grouped.columns if c in ALLOWED_311_SUMMARY_COLS or c in {"GEOID","date","hour_range","311_request_count"}]]
    ...
    try:
        ...
        # crime tarafı hour_range/hr_key
        ...
        if has_date:
            ...
            # Suffix'i engellemek için yalnızca gerekli kolonları seç
            summary_sel = summary[["GEOID", "date", "hour_range", "311_request_count"]].copy()
            overlap = (set(summary_sel.columns) & set(crime.columns)) - set(keys)
            if overlap:
                # sadece 311_request_count kaldığından teorik olarak overlap olmaz; yine de korumalı
                summary_sel = summary_sel.drop(columns=list(overlap - {"311_request_count"}), errors="ignore")

            merged = crime.merge(summary_sel, on=keys, how="left")
            ...
            # suffix önleme: cal_agg sadece anahtarlar + 311_request_count içerir
        ...
        # _x/_y kalmışsa güvence temizliği
        suffix_cols = [c for c in merged.columns if c.endswith("_x") or c.endswith("_y")]
        if suffix_cols:
            print(f"ℹ️ Güvence: suffix'li kolonlar temizleniyor: {suffix_cols}")
