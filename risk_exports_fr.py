...
"""
risk_exports_fr.py
FR veri hattı için saatlik + günlük risk exportları + opsiyonel Top-3 suç türü.

Girdi varsayılanı:
  - CRIME_DIR/fr_crime.csv  (Top-3 ve baseline için)
  - export_risk_tables(df, y, proba, threshold, out_prefix="") çağrısında
    'df' içinde en azından GEOID, date/hour_range (veya event_hour/datetime) olmalı.
Çıktılar:
  - {CRIME_DIR}/risk_hourly{out_prefix}.csv
  - {CRIME_DIR}/risk_daily{out_prefix}.csv
  - {CRIME_DIR}/patrol_recs{out_prefix}.csv  (aynı güne TOP_K öneriler)
  - Opsiyonel: {CRIME_DIR}/risk_types_top3.csv
"""
...
def _normalize_hour_range(hr):
    ...
    a = int(float(m.iloc[0, 0]))
    b = int(float(m.iloc[0, 1]))
    return f"{a:02d}-{b:02d}"


def _ensure_date_hour_on_df(df):
    """date ve hour_range alanlarını üret/normalize et; GEOID string kalsın."""
    df = df.copy()
    ...
    if "date" in df.columns:
        ...
        # FR hattında tarih zorunlu değilse günü "şimdi" ile doldur
        df["date"] = datetime.now(ZoneInfo("Europe/Paris")).date().isoformat()
...
def _load_recent_crime_fr(window_days=30):
    """Top-3 ve baseline için FR ham veri (fr_crime.csv) penceresi."""
    raw_path = os.path.join(CRIME_DIR, "fr_crime.csv")
...
def _compute_baselines_fr(window_days=30):
    """
    Saatlik bazda Poisson yoğunluğu (lambda) ve p=1-exp(-lambda) baz çizgileri;
    ayrıca kategori payları. Veri yoksa boş mapping döner.
    """
    dfc = _load_recent_crime_fr(window_days=window_days)
    ...
    # Şehir geneli hour_range bazında fallback
    ...
    # Şehir geneli kategori payları (hour_range)
...
def export_risk_tables(df, y, proba, threshold, out_prefix=""):
    """
    Saatlik + günlük risk çıktıları üretir (FR):
    ...
    # Aynı (GEOID, date, hour_range) için tek kayıt
    ...
    # FR baseline'ları ile beklenen değer ve Top-3 kategori tahsisi
    WINDOW_DAYS = int(os.getenv("TOP3_WINDOW_DAYS", "30"))
    lam_map, p_map, shares_map, city_fb, _ = _compute_baselines_fr(window_days=WINDOW_DAYS)
    ...
    for _, row in risk.iterrows():
        ...
        adj = _clip(adj, 0.2, 3.0)  # çok uçları kırp
    ...
    # Tarihi tekrar normalize (string)
    ...
    # Günlük birleşik olasılık (1 - ∏(1-p_hr))
    ...
    # Günlük Top-3 kategori (λ toplamına göre)
    _stack = pd.concat(
        [
    ...
    # Yazım hatası düzeltmesi (yanlış virgül boşluğu varsa toparla)
    if "date, " in _stack.columns:
        _stack = _stack.rename(columns={"date, ": "date"})
...
def optional_top_crime_types(window_days: int = 365, out_name: str = "risk_types_top3.csv"):
    ...
    """
    try:
        df = _load_recent_crime_fr(window_days=window_days)
        if df is None or len(df) == 0 or "category" not in df.columns:
            return None

        # overall
        over = (
            df.groupby("category").size().rename("count").reset_index().sort_values("count", ascending=False).head(3)
        )
        ...
        # hour_range bazında
        df["hour_range"] = df["hour_range"].apply(_normalize_hour_range)
        by_hr = df.groupby(["hour_range", "category"]).size().rename("count").reset_index()
        ...
        hr_top = (
            ...
            if rows
            else pd.DataFrame(columns=["hour_range", "category", "count", "rank", "scope"])
        )
        ...
        out[
            ["scope", "hour_range", "rank", "category", "count", "share_overall", "period_start", "period_end", "ndays"]
        ].to_csv(out_path, index=False)
        print(f"Top crime types (FR) → {out_path}")
        return out_path
    except Exception as e:
        print(f"[WARN] optional_top_crime_types(FR) failed: {e}")
        return None
...
def main():
    ...
    # CLI'dan çağrılırsa Top-3'ü de üretmeyi dene
